...
/**
 * Process the RPUSH command (see https://redis.io/commands/rpush).
 * @param key The key of the list.
 * @param value The assigned value.
 * @return index in list.
 */
int Redis::rpush(const char *key, const char *value)
{
    this->conn.println("*3");
    this->conn.println("$5");
    this->conn.println("RPUSH");
    this->conn.print("$");
    this->conn.println(strlen(key));
    this->conn.println(key);
    this->conn.print("$");
...
/**
 * Process the TIME command (see https://redis.io/commands/time).
 * @return UNIX time from Redis Server as String "UnixTime.MiliSeconds".
 */

String Redis::time(void) 
{
    this->conn.println("*1");
    this->conn.println("$4");
    this->conn.println("TIME");
    ...
    int startUnixTime = resp.indexOf("$10\r\n");
    String unixTime = resp.substring(startUnixTime + 5, startUnixTime + 5 + 10);
    String miliSecondZeroAppender = "000000";
    int UnixTimeMsecNumberOfDigits = resp.substring(startUnixTime + 5 + 10 + 2 + 1, startUnixTime + 5 + 10 + 2 + 1 + 1).toInt();
    int startUnixTimeMsec = resp.indexOf("$" + String(UnixTimeMsecNumberOfDigits) + "\r\n");
    String unixTimeMs = resp.substring(startUnixTimeMsec + 4, startUnixTimeMsec + 4 + UnixTimeMsecNumberOfDigits);
    return unixTime + "." + miliSecondZeroAppender.substring(0,6-UnixTimeMsecNumberOfDigits) + unixTimeMs;
}
